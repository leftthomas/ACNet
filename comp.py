...
if __name__ == '__main__':
    ...
    norm_embeds = torch.concat((norm_sketches, norm_photos), dim=0)
    triplet_embeds = torch.concat((triplet_sketches, triplet_photos), dim=0)
    norm_gan_embeds = torch.concat((norm_gan_sketches, norm_gan_photos), dim=0)
    triplet_gan_embeds = torch.concat((triplet_gan_sketches, triplet_gan_photos), dim=0)

    norm_embeds = tsne.fit_transform(norm_embeds.numpy())
    triplet_embeds = tsne.fit_transform(triplet_embeds.numpy())
    norm_gan_embeds = tsne.fit_transform(norm_gan_embeds.numpy())
    triplet_gan_embeds = tsne.fit_transform(triplet_gan_embeds.numpy())
