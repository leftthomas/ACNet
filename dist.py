...
if __name__ == '__main__':
    ...
    for paths, embeds in zip([old_paths, new_paths, photo_paths], [old_sketches, new_sketches, photos]):
        for path in tqdm(paths, desc='processing data'):
            emd = transform(Image.open(path)).flatten()
            embeds.append(emd)
    ...
    op_distance = ((old_sketches - photos) ** 2).sum(dim=-1).numpy()
    np_distance = ((new_sketches - photos) ** 2).sum(dim=-1).numpy()

    sns.set()
    data = {'Without Domain Elimination': op_distance, 'With Domain Elimination': np_distance}
